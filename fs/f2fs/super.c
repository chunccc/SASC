...
enum {
	...
	SUSPEND_GC_THREAD,	/* strcut f2fs_suspend_gc_trhead */
#endif
};
...
#ifdef CONFIG_F2FS_FS_SUSPEND_GC
	else if (struct_type == SUSPEND_GC_THREAD)
		return (unsigned char *)sbi->suspend_gc_thread;
#endif
	return NULL;
}
...
#ifdef CONFIG_F2FS_FS_SUSPEND_GC
	if (*ui) {
		wake_up_process(sbi->suspend_gc_thread->f2fs_suspend_gc_task);
	}
#endif
	return count;
}
...
#ifdef CONFIG_F2FS_FS_SUSPEND_GC
F2FS_RW_ATTR(SUSPEND_GC_THREAD, f2fs_suspend_gc_kthread, gc_suspend, state);
#endif
